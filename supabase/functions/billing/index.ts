...
serve(async (req) => {
  ...
  try {
    ...
    } else if (path.endsWith('/create-portal-session') && req.method === 'POST') {
...
async function getSubscription(req: Request, user: any) {
  try {
    ...
    const priceIdToPlan: { [key: string]: string } = {
      [Deno.env.get('VITE_STRIPE_PROFESSIONAL_MONTHLY_PRICE_ID') || '']: 'professional',
      [Deno.env.get('VITE_STRIPE_PROFESSIONAL_YEARLY_PRICE_ID') || '']: 'professional_yearly',
      [Deno.env.get('VITE_STRIPE_ENTERPRISE_MONTHLY_PRICE_ID') || '']: 'enterprise',
      [Deno.env.get('VITE_STRIPE_ENTERPRISE_YEARLY_PRICE_ID') || '']: 'enterprise_yearly',
    }
