...
2.  **Builder (Admin n8n):**
    -   Calls DigitalOcean API to `Create Droplet`.
...
4.  **The Reaper:** On `subscription.deleted` webhook, the Admin n8n calls `Destroy Droplet`.
...
*   **Risk:** A bad actor uses the server to spam or mine crypto. DigitalOcean bans the *entire* reseller account.
*   **Mitigation:**
