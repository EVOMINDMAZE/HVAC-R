---
name: Cloud Strategy
description: 100% serverless architecture with zero ops, infinite scale, pay-per-use philosophy. Local development uses Docker containers via supabase start.
version: 1.0
---
...
> **Note on Local Development**: While production is 100% serverless, local development uses Docker containers via `supabase start` for rapid iteration and testing. This maintains the serverless philosophy in production while enabling efficient local development.

## 1. Core Architecture

The HVAC-R SaaS Platform is built entirely on serverless primitives. We do not manage virtual machines (VMs) or containers in production.
