...
export const authenticateSupabaseToken: RequestHandler = async (
  ...
  try {
    ...
    const isMissingOrInsecure = !jwtSecret || jwtSecret === "your_super_secret_jwt_key_change_in_production" || jwtSecret === "fallback-secret-change-in-production";

    if (isProd && isMissingOrInsecure) {
      console.error("CRITICAL SECURITY ERROR: Missing or insecure JWT_SECRET in production environment. Failing securely.");
      return res.status(500).json({
        error: "Internal server configuration error",
      });
    }

    if (isMissingOrInsecure) {
