    ...
    try {
      ...
      if (supabase) {
        try {
          ...
          if (!configured) {
            throw new Error(
              "Supabase is not configured for browser use. Please set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY.",
            );
          }
          if (!isValidUrl) {
            throw new Error(
              `VITE_SUPABASE_URL appears invalid: ${supabaseUrl}`,
            );
          }

          const url = supabaseUrl.endsWith("/")
            ? supabaseUrl
            : `${supabaseUrl}/`;
          const controller = new AbortController();
