...
import { MultiCompanyAuthContext } from "./supabaseAuth.context";
import {
  UserRole,
  UserCompany,
  ActiveCompanyContext,
  MultiCompanyAuthContextType,
  COMPANIES_CACHE_TTL,
...
export const SupabaseAuthProvider = ({
  ...
  const clearCompanyCacheForUser = useCallback((userId: string | null | undefined) => {
    if (!userId) {
      return;
    }
    try {
      localStorage.removeItem(`companies_cache_${userId}`);
    } catch (_error) {
      return;
    }
  }, []);

  const clearAllCompanyCaches = useCallback(() => {
    try {
      const keysToRemove: string[] = [];
      for (let i = 0; i < localStorage.length; i += 1) {
        const key = localStorage.key(i);
        if (key?.startsWith("companies_cache_")) {
  ...
  const fetchCompanies = useCallback(async (currentUser?: User | null): Promise<UserCompany[]> => {
    ...
    try {
      ...
      if (cached) {
        ...
        const now = Date.now();
        if (Array.isArray(data) && now - timestamp < COMPANIES_CACHE_TTL) {
  ...
  const updateActiveCompany = useCallback(
    async (
      ...
      // If metadataCompanyId exists, treat that as manual selection (user already selected a company)
      const hasManualSelection = manuallySelected || !!metadataCompanyId;
      const needsSelection = companyList.length > 1 && !hasManualSelection;
      console.log("[useSupabaseAuth] updateActiveCompany debug:", { 
      ...
      setNeedsCompanySelection(needsSelection);
    },
    [],
  );
  ...
  const switchCompany = useCallback(
    async (
      ...
      try {
        ...
        // FIX: Refresh session to sync updated metadata
        const { error: refreshError } = await supabase.auth.refreshSession();
        if (refreshError) {
          console.warn("[MultiCompany] Session refresh warning:", refreshError);
          // Continue anyway - RPC succeeded, metadata will sync eventually
        }

        // Get updated user metadata to ensure immediate consistency
        const { data: userData } = await supabase.auth.getUser();
        const { data: sessionData } = await supabase.auth.getSession();
        const updatedUser = userData?.user;
        const updatedSession = sessionData?.session;
        
        // Update local state immediately to reflect metadata changes
        if (updatedUser) {
          setUser(updatedUser);
        }
        if (updatedSession) {
        ...
        clearCompanyCacheForUser(user.id);
        await refreshCompanies(true, updatedUser || user);
        return { success: true };
      } catch (err: any) {
    ...
    [clearCompanyCacheForUser, user, refreshCompanies],
  );
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (
        ...
        clearCompanyCacheForUser(previousUserIdRef.current);
      }
      if (!incomingUserId && previousUserIdRef.current) {
        clearCompanyCacheForUser(previousUserIdRef.current);
      }
      previousUserIdRef.current = incomingUserId;
