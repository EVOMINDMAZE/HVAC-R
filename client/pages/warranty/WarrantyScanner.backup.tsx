...
export default function WarrantyScanner() {
    ...
    const handleSaveAsset = async () => {
        ...
        if (!extractedData.serialNumber) {
            toast.error("No serial number detected.");
            return;
        }
        ...
        try {
            ...
            if (image) {
                ...
                const { data: uploadData, error: uploadError } = await supabase.storage
                    .from('asset-photos') // Ensure bucket exists or handle error
                    .upload(fileName, await (await fetch(image)).blob());

                if (uploadData) {
                    const { data: publicUrl } = supabase.storage.from('asset-photos').getPublicUrl(fileName);
            ...
            const { error } = await supabase.from('assets').insert({
                ...
                name: `${extractedData.manufacturer} Unit ${extractedData.modelNumber || ''}`,
                type: 'HVAC',
                serial_number: extractedData.serialNumber,
                model_number: extractedData.modelNumber,
                manufacturer: extractedData.manufacturer,
                photo_url: photoUrl,
                location_on_site: "Scanned via App"
            });

            if (error) throw error;
            toast.success("Asset saved to client profile!");
            reset();
                            ...
                            <div className="pt-4 border-t border-slate-100 dark:border-slate-800 space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <Button
                                        ...
                                        disabled={!extractedData.serialNumber || !selectedClientId || saving}
                                    >
