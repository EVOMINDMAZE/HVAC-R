...
import { Loader2, CheckCircle, AlertTriangle, ArrowRight, UserPlus, FileText, Phone, MessageSquare, Eye, Archive, Clock, ShieldCheck, Zap, ImageIcon } from "lucide-react";
...
export default function TriageDashboard() {
    ...
    useEffect(() => {
        ...
        const timeoutId = setTimeout(() => {
            if (loading) {
                console.warn('[TriageDashboard] Query timeout - forcing loading to false');
                setLoading(false);
            }
    ...
    const handleArchive = async (id: string) => {
        try {
            ...
            if (error) throw error;
            toast({ title: "Lead Archived", description: "Submissions moved to archives." });
            setSelectedSubmission(null);
        } catch (err: any) {
    ...
    const getSeverityBadge = (severity?: string) => {
        if (severity === 'high') return <Badge variant="destructive" className="animate-pulse">Emergency</Badge>;
        if (severity === 'medium') return <Badge className="bg-orange-500">Urgent</Badge>;
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 p-4 md:p-8 pb-20">
            ...
            <Dialog open={!!selectedSubmission} onOpenChange={(open) => !open && setSelectedSubmission(null)}>
                <DialogContent className="max-w-4xl max-h-[90vh] p-0 overflow-hidden flex flex-col border-none shadow-2xl">
                    {selectedSubmission && (
                        <>
                            <div className="bg-gradient-to-r from-blue-700 to-indigo-800 p-6 text-white pb-12">
                                <DialogHeader>
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex items-center gap-2">
                                            <Zap className="h-5 w-5 fill-white text-white" />
                                            <span className="text-xs font-bold uppercase tracking-[0.2em] opacity-80">AI Diagnostic Report</span>
                                        </div>
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                                        <div>
                                            <DialogTitle className="text-3xl font-black">{selectedSubmission.homeowner_name}</DialogTitle>
                                            <div className="flex items-center gap-4 mt-2 opacity-80 text-sm font-medium">
                                                <span className="flex items-center gap-1.5"><Phone className="h-3.5 w-3.5 fill-white" /> {selectedSubmission.homeowner_phone}</span>
                                                <span className="flex items-center gap-1.5"><Clock className="h-3.5 w-3.5" /> Received {new Date(selectedSubmission.created_at).toLocaleString()}</span>
                                            </div>
                                        </div>
                                        {selectedSubmission.status !== 'converted' && selectedSubmission.status !== 'archived' && (
                                            <div className="flex gap-2">
                                                ...
                                                >
                                                    {converting === selectedSubmission.id ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <UserPlus className="h-4 w-4 mr-2" />}
                                                    Convert to Full Job
                                                </Button>
                                            </div>
                                        )}
                            ...
                            <ScrollArea className="flex-1 p-6 relative -mt-6 bg-slate-50 dark:bg-slate-900 rounded-t-3xl">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                    <div className="md:col-span-2 space-y-8">
                                            ...
                                            <div className="grid grid-cols-1 gap-4">
                                                <Card className="bg-blue-600 text-white border-none shadow-lg">
                                                    <CardContent className="p-6">
                                                        ...
                                                        <p className="text-xl font-bold leading-tight">
                                                            {selectedSubmission.ai_analysis?.suspected_issue || "AI analysis pending..."}
                                                        </p>
                                                    </CardContent>
                                                </Card>
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border">
                                                        <span className="text-[10px] font-bold text-slate-400 block mb-1 uppercase tracking-wider">Severity Status</span>
                                                        <div className="flex items-center gap-2">
                                                            {getSeverityBadge(selectedSubmission.ai_analysis?.severity)}
                                                            <span className="text-sm font-bold opacity-60 capitalize">{selectedSubmission.ai_analysis?.severity} Risk</span>
                                                        </div>
                                                    </div>
                                                    <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border">
                                                        <span className="text-[10px] font-bold text-slate-400 block mb-1 uppercase tracking-wider">Equipment Identified</span>
                                                        <p className="text-sm font-bold text-slate-700 dark:text-slate-200">
                                                            {selectedSubmission.ai_analysis?.equipment_details || "General HVAC Unit"}
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <section className="space-y-4">
                                            <h3 className="text-sm font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                                                <MessageSquare className="h-4 w-4" /> Homeowner Description
                                            </h3>
                                        ...
                                        {/* Evidence Grid */}
                                        {selectedSubmission.media_urls && selectedSubmission.media_urls.length > 0 && (
                                            <section className="space-y-4 pb-8">
                                                <h3 className="text-sm font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                                                    <ImageIcon className="h-4 w-4" /> Photographic Evidence
                                                </h3>
                                                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                                    {selectedSubmission.media_urls.map((url, i) => (
                                                        <div key={i} className="group relative rounded-xl overflow-hidden aspect-square border-2 border-transparent hover:border-blue-500 transition-all cursor-zoom-in">
                                            ...
                                            <div className="text-sm leading-relaxed text-slate-700 dark:text-slate-300 space-y-4">
                                                {selectedSubmission.ai_analysis?.technician_notes ? (
                                                    <div className="space-y-3">
                                                        {selectedSubmission.ai_analysis.technician_notes.split('\n').map((note, idx) => (
                                                            <div key={idx} className="flex gap-2">
                                                                <div className="min-w-[4px] h-[4px] bg-blue-500 rounded-full mt-2" />
                                                                <span>{note}</span>
                                                            </div>
                                                        ))}
                                                    </div>
                                                ) : (
                                                    <p className="italic opacity-60 text-xs">AI notes being finalized...</p>
                                                )}
                                            </div>
                                        </div>

                                        <div className="bg-white dark:bg-slate-950 p-5 rounded-xl border border-blue-100 dark:border-blue-900 shadow-sm">
                                            <h4 className="text-[10px] font-black uppercase text-blue-600 dark:text-blue-400 mb-2">Dispatcher Action</h4>
                                            <p className="text-xs text-slate-500 mb-4 font-medium italic">
                                                Convert this lead to a Job to assign to a tech and schedule site visit.
                                            </p>
                                            <Button
                                                className="w-full bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-500/20 h-10 font-bold text-xs"
                                                onClick={() => handleConvert(selectedSubmission)}
                                                disabled={converting === selectedSubmission.id || selectedSubmission.status === 'converted'}
                                            >
                                                Convert to Full Job
                                            </Button>
                                        </div>
