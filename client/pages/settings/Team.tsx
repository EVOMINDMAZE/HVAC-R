...
import { Loader2, Mail, Trash2, Shield, UserPlus, ArrowLeft } from "lucide-react";
...
export default function Team() {
    const { user, role: myRole } = useSupabaseAuth();
    ...
    const fetchTeam = async () => {
        try {
            const { data: teamData, error } = await supabase.rpc('get_company_team');
    ...
    const handleRoleUpdate = async (userId: string, newRole: 'admin' | 'manager' | 'tech') => {
        try {
    ...
    const handleDeleteUser = async (userId: string) => {
        if (!confirm("Are you sure you want to remove this team member? This will delete their role mapping.")) return;
        try {
            const { error } = await supabase
                .from('user_roles')
                .delete()
        ...
        <PageContainer variant="standard" className="space-y-8">
            <div className="flex justify-between items-center">
                <div className="space-y-1">
                    ...
                    <p className="text-muted-foreground">Manage your technicians and office staff.</p>
                </div>
            </div>
                                    ...
                                    {members.map((member) => (
                                        <TableRow key={member.user_id}>
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <span className="font-medium text-sm">
                                                            {member.user_id === user?.id ? "You" : (member.email || "User")}
                                                        </span>
                                            ...
                                            <TableCell>
                                                {member.user_id === user?.id ? (
                                                    ...
                                                    <Select
                                                        ...
                                                        disabled={!(myRole === 'admin' || (myRole === 'manager' && member.role !== 'admin'))}
                                                    >
                                            ...
                                            <TableCell className="text-right">
                                                {(myRole === 'admin' || (myRole === 'manager' && member.role !== 'admin')) && member.user_id !== user?.id && (
                                                    <Button
                                                        variant="ghost"
                                                        size="icon"
