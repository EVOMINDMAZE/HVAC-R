    ...
    <Card className="border-0 shadow-lg bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm overflow-hidden">
      <CardHeader className="bg-blue-50/50 dark:bg-slate-800/50 border-b border-blue-100 dark:border-slate-700 pb-4">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className="text-blue-600 border-blue-200 bg-blue-50 hidden sm:flex">Q = 1.08 × CFM × ΔT</Badge>
        </div>
      </CardHeader>
          ...
          <div className="p-6 md:p-8 space-y-8 border-r border-slate-100 dark:border-slate-800">
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <Label className="text-sm font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider text-xs">Based on Heat Load</Label>
                <div className="flex items-center gap-3">
                  <div className="relative flex-1 group">
                    ...
                    <Input
                      ...
                      onChange={(e) => setInputs({ ...inputs, sensible_heat: Number(e.target.value) })}
              ...
              <div className="space-y-3">
                <Label className="text-sm font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider text-xs">Measured Temp. Split (ΔT)</Label>
                <div className="flex items-center gap-3">
                  <div className="relative flex-1 group">
                    ...
                    <Input
                      ...
                      onChange={(e) => setInputs({ ...inputs, delta_t: Number(e.target.value) })}
                      className="pl-9 h-11 bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-700 group-hover:border-blue-400 transition-colors"
                    />
                  </div>
              ...
              <div className="pt-4">
                <Button
                  ...
                  className="w-full h-12 text-base bg-blue-600 hover:bg-blue-700 shadow-xl shadow-blue-500/20"
                  disabled={loading}
                >
                  {loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Calculate Required Airflow"}
                </Button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50/80 dark:bg-slate-900/50 p-6 md:p-8 flex flex-col justify-center items-center relative min-h-[300px]">
            ...
            {result ? (
              <div className="text-center w-full max-w-sm animate-in zoom-in-95 duration-300">
                <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 mb-6 shadow-inner">
                  <Wind className="w-10 h-10" />
                </div>
                <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-widest mb-2">Required Airflow</h3>
                ...
                <div className="grid grid-cols-2 gap-4 text-xs text-muted-foreground border-t border-slate-200 dark:border-slate-800 pt-6">
                  <div>
                    ...
                    <strong className="text-slate-700 dark:text-slate-300">{inputs.sensible_heat} {units.sensible_heat}</strong>
                  </div>
                  <div>
                    <span className="block mb-1">Target ΔT</span>
                    <strong className="text-slate-700 dark:text-slate-300">{inputs.delta_t}°{units.delta_t}</strong>
                  </div>
                </div>
                ...
                <div className="mt-8">
                  <SaveCalculation
                    ...
                    results={result}
                    trigger={
                      <Button variant="outline" className="w-full border-dashed border-slate-300 hover:border-blue-500 hover:text-blue-600 transition-all">
              ...
              <div className="text-center text-slate-400 py-12">
                ...
                <p className="max-w-[200px] mx-auto text-sm">Enter sensible heat and temperature split to calculate required airflow</p>
              </div>
            )}
          </div>
        </div>
...
function DeltaTCalculator() {
  ...
  if (result) {
    ...
    else if (units.result === "K") displayDeltaT = (result.delta_t_f / 1.8).toFixed(1);
      ...
      <CardHeader className="bg-emerald-50/50 dark:bg-slate-800/50 border-b border-emerald-100 dark:border-slate-700 pb-4">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <CardDescription>
              Measure temperature drop across the evaporator coil to verify performance.
            </CardDescription>
          </div>
        </div>
      </CardHeader>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2">
          ...
          <div className="p-6 md:p-8 space-y-8 border-r border-slate-100 dark:border-slate-800">
            <div className="space-y-6">
              {/* Supply */}
              <div className="space-y-3">
                <Label className="text-sm font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider text-xs">Return Air (Intake)</Label>
                <div className="flex items-center gap-3">
                  <div className="relative flex-1 group">
                    <span className="absolute left-3 top-2.5 text-slate-400">↩️</span>
                    <Input
                      ...
                      onChange={(e) => setInputs({ ...inputs, return_temp: Number(e.target.value) })}
              ...
              {/* Return */}
              <div className="space-y-3">
                <Label className="text-sm font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider text-xs">Supply Air (Output)</Label>
                <div className="flex items-center gap-3">
                  <div className="relative flex-1 group">
                    <span className="absolute left-3 top-2.5 text-slate-400">💨</span>
                    <Input
                      ...
                      onChange={(e) => setInputs({ ...inputs, supply_temp: Number(e.target.value) })}
                      className="pl-9 h-11 bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-700 group-hover:border-emerald-400 transition-colors"
                    />
                  </div>
          ...
          <div className="bg-slate-50/80 dark:bg-slate-900/50 p-6 md:p-8 flex flex-col justify-center items-center relative min-h-[300px]">
            ...
            {result ? (
              <div className="text-center w-full max-w-sm animate-in zoom-in-95 duration-300">
                ...
                <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-widest mb-2">Measured Difference</h3>
                <div className="flex items-baseline justify-center gap-1 mb-2">
                  <span className={`text-6xl font-black tracking-tighter ${result.status.includes("Normal") ? "text-slate-900 dark:text-white" : "text-amber-600 dark:text-amber-400"}`}>
                    {displayDeltaT}
                  </span>
                  <span className="text-lg text-slate-400 font-medium">
                ...
                <Badge variant={result.status.includes("Normal") ? "default" : "secondary"} className={`mb-8 ${result.status.includes("Normal") ? "bg-emerald-600 hover:bg-emerald-600" : "bg-amber-100 text-amber-700 hover:bg-amber-200"}`}>
                  {result.status}
                </Badge>
                ...
                <div className="mt-4">
                  <SaveCalculation
                    ...
                    results={result}
                    trigger={
                      <Button variant="outline" className="w-full border-dashed border-slate-300 hover:border-emerald-500 hover:text-emerald-600 transition-all">
                        <Save className="w-4 h-4 mr-2" />
                        Save Result
                      </Button>
                    }
              ...
              <div className="text-center text-slate-400 py-12">
                ...
                <p className="max-w-[200px] mx-auto text-sm">Enter return and supply air temperatures to check system performance</p>
              </div>
            )}
          </div>
        </div>
