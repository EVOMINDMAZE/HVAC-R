                ...
                <Card>
                    <CardContent className="p-4 flex flex-col items-center justify-center text-center">
                        <div className="text-2xl font-bold text-green-600">
                            {cylinders.reduce((acc, c) => acc + (c.status === 'active' ? Number(c.current_weight_lbs) : 0), 0).toFixed(1)}
                        </div>
            ...
            <div className="grid gap-4">
                {loading ? (
                    ...
                    cylinders.map((cylinder) => (
                        ...
                        >
                            <Card className="overflow-hidden hover:shadow-md transition-shadow cursor-pointer border-l-4 border-l-blue-500">
                                <CardContent className="p-4">
                                    <div className="flex items-center justify-between mb-4">
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-2">
                                                <h3 className="font-bold text-lg">{cylinder.refrigerant_type}</h3>
                                                <Badge variant="secondary" className="font-mono text-xs">
                                                    {cylinder.cylinder_code}
                                                </Badge>
                                            </div>
                                            <div className="text-sm text-muted-foreground">
                                                Weight: <span className="font-medium text-foreground">{cylinder.current_weight_lbs} lbs</span> / {cylinder.initial_weight_lbs} lbs
                                            </div>
                                        </div>
                                    ...
                                    <LogRefrigerantDialog
                                        ...
                                        onLogSuccess={() => {
                                            fetchCylinders();
                                            toast({ title: "Updated", description: "Inventory usage recorded." });
                                        }}
                                    />
                                </CardContent>
                            </Card>
