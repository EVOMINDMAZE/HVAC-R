...
export default function ComplianceReport() {
    ...
    const handleExportCSV = () => {
        ...
        link.setAttribute("download", `EPA_608_Log_${format(new Date(), 'yyyyMMdd')}.csv`);
    ...
    const getActionColor = (type: string) => {
        switch (type) {
            case "charge": return "bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-200";
            case "recover": return "bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-200";
            case "addition": return "bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-200";
            case "disposal": return "bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-200";
            default: return "";
        }
    };
        ...
        <div className="container mx-auto max-w-6xl p-4 space-y-6 pb-24">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex gap-2">
                    <Button onClick={handleExportCSV} disabled={logs.length === 0 || loading} variant="outline">
                                        ...
                                        <TableRow key={log.id}>
                                            ...
                                            <TableCell>
                                                <div className="flex flex-col">
                                                    <span className="font-medium">{log.cylinder?.cylinder_code}</span>
                                                    <span className="text-xs text-muted-foreground">{log.cylinder?.refrigerant_type}</span>
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                <Badge variant="outline" className={`capitalize ${getActionColor(log.transaction_type)} border-0`}>
                                                    {log.transaction_type}
                                                </Badge>
                                            </TableCell>
