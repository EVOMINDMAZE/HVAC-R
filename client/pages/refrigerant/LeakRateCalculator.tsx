...
import { ArrowLeft, Calculator, AlertTriangle, CheckCircle, Info, History, Database } from "lucide-react";
...
export default function LeakRateCalculator() {
    ...
    const calculateRate = (logData: LogEntry[]) => {
        ...
        if (logData.length > 1) {
            days = differenceInDays(new Date(latestAddition.created_at), new Date(logData[1].created_at));
        } else {
    ...
    const getThresholdBadge = (rate: number) => {
        ...
        return <Badge variant="outline" className="text-green-600 border-green-200">Stable</Badge>;
    };
        ...
        <div className="container mx-auto max-w-4xl p-4 space-y-6 pb-24">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Only assets with a defined "Full Charge" are shown.</CardDescription>
                </CardHeader>
                <CardContent>
                        ...
                        <SelectContent>
                            {assets.map(asset => (
                                <SelectItem key={asset.id} value={asset.id}>
                                    {asset.name} ({asset.refrigerant_type}) - S/N: {asset.serial_number || 'Unknown'}
                                </SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                    {!loading && assets.length === 0 && (
                        <p className="text-sm text-amber-600 mt-2 flex items-center gap-1">
                            ...
                            No assets found with design charge data. Update equipment details first.
                        </p>
                    )}
                </CardContent>
            </Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            {leakRate !== null ? (
                                <>
                                    <div className="text-center py-8">
                                        <div className={`text-6xl font-black mb-2 ${getThresholdColor(leakRate)}`}>
                                            {leakRate.toFixed(1)}%
                                        </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-sm bg-muted/50 p-4 rounded-lg">
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-muted-foreground">Full Design Charge</p>
                                            <p className="font-bold">{selectedAsset.full_charge_lbs} lbs</p>
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-muted-foreground">Days Since Last Check</p>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Progress value={Math.min(leakRate * 3.33, 100)} className="h-1" />
                                        {leakRate > 10 && (
                                ...
                                <div className="text-center py-20 text-muted-foreground">
                                    ...
                                    <p className="text-xs mt-2">Logs must be of type "Charge" and linked to this asset.</p>
                                </div>
                            )}
                        </CardContent>
                    </Card>
                    ...
                    <Card className="bg-muted/30 border-none shadow-none">
                        ...
                        <CardContent className="text-xs space-y-4 text-muted-foreground leading-relaxed">
                            <p>
                                The **Annualizing Method** is one of two EPA-approved ways to calculate leak rates.
                            </p>
                            <div className="p-3 bg-background rounded border font-mono">
                                Rate = (lbs / charge) * (365 / days)
                            </div>
                            <p className="font-bold text-foreground">Compliance Thresholds:</p>
                            ...
                            <p className="italic">
                                If the leak rate exceeds these thresholds, the EPA requires repair within 30 days.
                            </p>
                        </CardContent>
                    </Card>
                </div>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="space-y-3">
                            {logs.map((log) => (
                                <div key={log.id} className="flex items-center justify-between p-3 border rounded-lg bg-background hover:bg-muted/30 transition-colors">
