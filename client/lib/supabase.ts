...
export const getSupabaseConfig = () => {
  ...
  const configured = Boolean(supabaseUrl && supabaseAnonKey) && !supabaseUrl.includes('your-supabase') && !supabaseAnonKey.includes('your-supabase');
  return {
    supabaseUrl,
    supabaseAnonKey,
    configured,
...
const createSupabaseClient = () => {
  ...
  if (!configured || !valid) {
    console.warn('Supabase environment variables not configured or invalid. Please set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY');
    return null;
  }
