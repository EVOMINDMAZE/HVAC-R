...
export function setupGlobalErrorHandler() {
  ...
  window.addEventListener("unhandledrejection", (event) => {
    ...
    if (
      ...
      console.warn(
        "A JSON parsing error was caught and prevented from breaking the app",
      );
    }
  });
  ...
  JSON.parse = function (text: string, reviver?: any) {
    try {
      ...
      const isFromExternalAPI =
        stack.includes("simulateon-backend-new.onrender.com") ||
      ...
      console.error("🚨 JSON.parse FAILED:", {
        ...
        source: isFromExternalAPI
          ? "External Calculation API"
          : isFromInternalAPI
            ? "Internal Backend API"
            : "Unknown",
        textPreview:
          typeof text === "string" ? text.substring(0, 200) : "Not a string",
      ...
      if (isFromExternalAPI) {
        console.warn(
          "💡 This appears to be from the external calculation API. Check if the API server is returning HTML error pages.",
        );
      } else if (isFromInternalAPI) {
        console.warn(
          "💡 This appears to be from the internal backend API. Check if the API server is configured and running.",
        );
      }
