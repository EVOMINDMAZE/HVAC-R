...
import { generateWinterizationCert, generateCommissioningCert, generateMaintenanceCert, downloadPdf } from '@/utils/pdfGenerator';
...
export function RiskShield() {
    ...
    const handleGenerate = async (type: 'winter' | 'commission' | 'maintenance') => {
        setGenerating(type);
        try {
            // Real context data
            ...
            const baseData = {
                ...
                technicianName: user?.user_metadata?.full_name || user?.email || "Verified Technician",
            ...
            if (type === 'winter') {
                ...
                bytes = await generateCommissioningCert({
                    ...baseData,
                    checklistItems: [
                        { label: "Refrigerant Charge Verification (Superheat/Subcooling)", passed: true },
        ...
        <div className="border rounded-lg p-6 bg-card shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Generate insurance-ready compliance reports</p>
                </div>
            </div>
                ...
                <div className="grid gap-3">
                    <Card className="p-3 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors cursor-pointer border-slate-100 dark:border-slate-800">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="space-y-0.5">
                                ...
                                <p className="text-xs text-muted-foreground">Seasonal readiness check</p>
                            </div>
                        </div>
                        <Button
                            variant="outline"
                            ...
                            disabled={!!generating}
                        >
                            {generating === 'winter' ? <Loader2 className="h-3 w-3 animate-spin" /> : <Download className="h-3 w-3" />}
                        </Button>
                    </Card>
                    ...
                    <Card className="p-3 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors cursor-pointer border-slate-100 dark:border-slate-800">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="space-y-0.5">
                                ...
                                <p className="text-xs text-muted-foreground">Start-up & verification</p>
                            </div>
                        </div>
                        <Button
                            variant="outline"
                            ...
                            disabled={!!generating}
                        >
                            {generating === 'commission' ? <Loader2 className="h-3 w-3 animate-spin" /> : <Download className="h-3 w-3" />}
                        </Button>
                    </Card>
                    ...
                    <Card className="p-3 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors cursor-pointer border-slate-100 dark:border-slate-800">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="space-y-0.5">
                                <p className="text-sm font-medium mr-2">Preventative Maintenance</p>
                                <p className="text-xs text-muted-foreground">System health check</p>
                            </div>
                        </div>
                        <Button
                            variant="outline"
                            ...
                            disabled={!!generating}
                        >
                            {generating === 'maintenance' ? <Loader2 className="h-3 w-3 animate-spin" /> : <Download className="h-3 w-3" />}
                        </Button>
                    </Card>
                </div>
