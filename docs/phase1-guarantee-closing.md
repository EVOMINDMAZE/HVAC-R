...
| Evodelta footer | strip + terms section | ✅ LIVE — strip in Footer.tsx L207, Terms §3 inserted, stale §16 "no payments processed" rewritten to live-checkout refund wording |
...
| Terms sections | 4 apps | ✅ Box §3, Evodelta §3, VanClass §3 strengthened, Cryovo §4 strengthened |
...
- **Evodelta**: portal ✅ / CSV export ✅ / 30-day ✅
...
- Evodelta: committed + `vercel --prod --yes` (tsc 0 errors, 115/115 tests)
...
- **Goal**: signed-in Box user lands on Evodelta (and ideally VanClass/Cryovo) and is recognized — or honestly one-clicks via magic link. Document the REAL SSO truth (same Supabase project ≠ shared cookies across different domains).
...
- **Gate**: recorded walkthrough — QA user signed into Box lands on Evodelta recognized (or one-click) — screenshot evidence; no fake SSO claims.
