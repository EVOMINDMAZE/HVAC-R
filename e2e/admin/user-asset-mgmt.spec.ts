...
test.describe("User & Asset Management", () => {
      ...
      if (response.url().includes("supabase.co")) {
        if (response.status() >= 400) {
          try {
            ...
            console.error(
              `[Error Response] ${response.url()}: ${response.status()}`,
            );
          }
        }
      }
  ...
  test("Admin can invite a technician and update their role", async ({
    page,
  }) => {
    ...
    const responsePromise = page.waitForResponse(
      (response) => response.url().includes("invite-user"),
      { timeout: 15000 },
    );
  ...
  test("Admin can edit an asset on a client page", async ({ page }) => {
    ...
    await expect(noAssetsMsg.or(assetCardLocator.first())).toBeVisible({
      timeout: 10000,
    });
    ...
    if (await noAssetsMsg.isVisible()) {
      ...
      await expect(page.getByText("Register New Asset")).toBeVisible({
        timeout: 10000,
      });
  ...
  test("Admin can invite a new client user", async ({ page }) => {
    ...
    await expect(page.getByText("Manage Client Access")).toBeVisible({
      timeout: 10000,
    });
