...
(async () => {
  ...
  for (const viewport of viewports) {
    ...
    const metrics = await page.evaluate(() => {
      ...
      const grid = hero?.querySelector("div.grid") || null;
      const columns = grid ? Array.from(grid.children) : [];
      const leftRail = columns[0] || null;
      const rightRail = columns[1] || null;
      ...
      const primaryCtaVisible = Boolean(hero?.querySelector("a[href='/signup']"));
      const secondaryCtaVisible = Boolean(
        hero?.querySelector("a[href='/demo'], a[href='/contact']"),
      );

      const rect = rightRail?.getBoundingClientRect?.() || null;
      const rightRailVisible = Boolean(rect && rect.width > 0 && rect.height > 0);
      ...
      return {
        ...
        secondaryCtaVisible,
        rightRailVisible,
        rightRailTop: rect ? rect.top : null,
        rightRailBottom: rect ? rect.bottom : null,
    ...
    results.push({
      ...
      rightRailMostlyAboveFold:
        metrics.rightRailVisible && typeof metrics.rightRailBottom === "number"
