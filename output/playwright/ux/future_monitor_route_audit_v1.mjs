...
async function captureRoute(page, route, bypass, expectVisible, skin = monitorSkin, viewport) {
  ...
  try {
    ...
    const monitorPresent = Boolean(data.hasMonitorShell || data.hasMonitorDock);

    if (!expectVisible && monitorPresent) {
      passed = false;
      reason = "monitor_ui_present_when_disabled";
    } else if (skin && monitorPresent && data.monitorShellSkin && data.monitorShellSkin !== skin) {
      passed = false;
      reason = "monitor_shell_skin_mismatch";
    } else if (skin && monitorPresent && data.monitorSkin && data.monitorSkin !== skin) {
      passed = false;
